...
const MiniCards = observer((props) => {
  ...
  async function getTotalBnbLocked(defiName) {
    ...
    const res = await fetch(
      global.defistationApiUrl + "/bnblockedList/" + urlStr,
      {
        method: "GET",
        headers: {
    ...
    res
      ...
      .then((res) => {
        ...
        // console.log("res: ", res);
        setTotalBnbLockedNum(
          numberWithCommas(Math.floor(resultArr[resultArr.length - 1][1]))
        );
        ...
        if (props.defiName != "DeFi") {
          ...
          setMiniCardData3(
            (
              ((resultArr[resultArr.length - 1][1] * 1) / 147883948) *
              100
            ).toFixed(4) *
              1 +
              "%"
        ...
        if (resultArr[resultArr.length - 1][0] == 0) {
          ...
          tempDate = new Date(resultArr[resultArr.length - 1][0] * 1000)
            .toISOString()
            .replace(/T/, " ")
            .replace(/\..+/, "");
